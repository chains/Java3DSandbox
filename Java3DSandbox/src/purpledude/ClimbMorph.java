...
public class ClimbMorph extends javax.media.j3d.Behavior {
	...
	private void moveStep() {
		if (rightDown || leftDown || fwDown) {
			...
			// checkTransform("    about to  translate ", ourTrans); debugging
			// code - left here as an example
			ourTrans.mul(tempTrans);
			// checkTransform("    after translation ", ourTrans); debugging
			// code - left here as an example
	...
	/*
	 * A method to transform a point from a transform3D and send the resulting x
	 * y and z values to System.err. This is left here, commented out, as an
	 * example of the kind of thing you may find it useful to do when you're
	 * having problems with your code. Clear and complete traces like this can
	 * be a real asset. As can a proper debugging tool, but lots of system.err
	 * calls are easier...
	 * 
	 * private void checkTransform(String note, Transform3D oatBlanket) {
	 * Point3d checkme = new Point3d(); oatBlanket.transform(checkme);
	 * System.err
	 * .println(note+" > x = "+checkme.x+" y = "+checkme.y+" z = "+checkme.z); }
	 */
